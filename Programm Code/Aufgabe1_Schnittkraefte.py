...
from Aufgabe1_Gleichgewichte import Aufgabe1_Gleichgewichte

api_key = "sk-proj..."
client = OpenAI(api_key=api_key)

class Aufgabe1_Schnittkraefte:

    def __init__(self, master=None):

        self.aufgabe = AufgabeSchnittkraefte("Wähle das korrekte Freikörperbild!", "A")

        self.master = master
        self.Aufg1_SchnittGUI = tk.Toplevel(master)
        self.Aufg1_SchnittGUI.resizable(False, False)
        self.Aufg1_SchnittGUI.title("Level 1.1 - Schnittkraefte")
        self.Aufg1_SchnittGUI.geometry("1280x720")
        ...
        self.canvas = tk.Canvas(self.Aufg1_SchnittGUI, width=1280, height=720, highlightthickness=0)
        ...
        self.canvas.create_text(830, 30, text="Level 1.1 - Freikörperbild", font=("Helvetica", 18, "bold"), fill="black")
        ...
        self.bildAufgabe = Image.open("SK1_Aufgabenstellung.png")
        ...
        # Button Menü
        self.btnBackToMenu = tk.Button(self.Aufg1_SchnittGUI, text="Menü", command=self.backToMenu, font=("Verdana", 12, "bold"), bg="white")
        ...
        for name in ["a", "b", "c"]:
            try:
                img = Image.open(f"SK1_Antwort_{name}.png").resize((200, 150))
                ...
                self.canvas.create_text(830, 600 + 30 * (ord(name) - ord('a')),
                                        text=f"Bild SK1_Antwort_{name}.png fehlt!", fill="red", font=("Helvetica", 10))
        ...
        for i, name in enumerate(["A", "B", "C"]):
            if self.bilder[name]:
                btn = tk.Button(self.Aufg1_SchnittGUI, image=self.bilder[name], command=lambda n=name: self.pruefe_aufgabe(n))
    ...
    def oeffneAufgabeGleichgewichte(self):
        Aufgabe1_Gleichgewichte(master=self.master)
        self.Aufg1_SchnittGUI.withdraw()

    def backToMenu(self):
        self.Aufg1_SchnittGUI.destroy()
        if self.master:
    ...
    def chatfenster_hinzufuegen(self, verlauf=""):
        ...
        self.chat_frame = tk.Frame(self.Aufg1_SchnittGUI, bg="#7b4be6", bd=1, relief=tk.GROOVE)
